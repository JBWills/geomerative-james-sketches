...
class RCommand(
   ...
   * @invisible
   */
  constructor(c: RCommand) : this(c.startPoint, c.endPoint, c.controlPoints.clone(), c.commandType)
  ...
  fun getPoints(resetSegmentator: Boolean): Array<RPoint> {
    ...
    when (segmentType) {
      ...
      UNIFORMLENGTH -> when (commandType) {
        LINETO -> {
          lineUniformLength()
          result = curvePoints
          curvePoints = arrayOf()
  ...
  override fun getPoint(t: Float): RPoint {
    ...
    return when (commandType) {
      ...
      else -> throw Exception("Can call getTangent with type: $commandType")
    }
  }
   ...
   * @invisible
   */
  fun getCurveLength(t: Float): Float = when (commandType) {
    LINETO -> {
   ...
   * @return RPoint[], the tangent vectors returned in an array.
   */
  fun split(t: Float): Array<RCommand> = when (commandType) {
    LINETO -> splitLine(t)
  ...
  fun intersectionPoints(other: RCommand): Array<RPoint> = when (commandType) {
    LINETO -> when (other.commandType) {
      ...
      else -> throw Exception("Invalid other command type: ${other.commandType}")
    }
    QUADBEZIERTO -> when (other.commandType) {
    ...
    CUBICBEZIERTO -> when (other.commandType) {
    ...
    else -> throw Exception("Invalid command type: $commandType")
  }
